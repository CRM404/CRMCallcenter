...
--
-- Сидинг ниже гоняется при каждом старте и потому идемпотентен по
-- (stage_number, status_name): заведённые руками строки он не трогает, а
-- удалённые из сидинга — воскрешает. Это осознанно: перечень сидинга и есть
-- тот минимум, на который опирается код.

...
CREATE TABLE IF NOT EXISTS real_estate_offers (
    ...
    mortgage_type VARCHAR,
    priority INTEGER,
    -- ⚠ КОЛОНКА ЖИВА, НО С ЭКРАНА СНЯТА (часть 9, заход 5). Поле «Время для
    -- перевода» было свободной строкой вроде «до 15 минут» — временем его
    -- считать нельзя, и переносить в событие «Перевод» нечего: там своё поле в
    -- секундах у каждой строки. Из карточки оффера и из ответа API поле убрано,
    -- в базе оставлено намеренно: снятие с экрана обратимо, удаление колонки —
    -- нет, и на него нужно отдельное слово владельца.
    --
    -- Замер на бою 27.08.2026 (`59434ba`): заполнено у 0 офферов из 39 — то есть
    -- снятие не потеряло ничего.
...
-- перезвон через час, двадцать попыток, окно 9–21, — становятся четырьмя

...
-- ЧИСЛА СЕГМЕНТА ПРОВЕРЯЕТ И БАЗА (К304). Правило простое и однострочное, а
-- маршрут — не единственная дверь: правка запросом мимо экрана попадает сюда же.
--
-- ⚠ ЧТО ИМЕННО ОТБИВАЕТСЯ: отрицательное значение в любой из четырёх колонок и
-- перевёрнутая пара. РАВЕНСТВО ЗАКОННО — «цена ровно 5 000 000» это точечный
-- отбор; верхнего предела нет намеренно (довод К269 и К270: обоснованного числа
-- не существует, а выдуманное упрётся в настоящий дорогой объект).
--
-- ⚠ ПРОВЕРКА ПАРЫ СМОТРИТ НА ОДНУ СТРОКУ, и потому выразима CHECK-ом — в
-- отличие от правила «одна сеть на номер» (К265), которое связывает ДВЕ строки
-- и живёт только в маршруте. Разница названа здесь, чтобы следующий не искал
-- симметрии там, где её нет.
--
-- БЕЗ `NOT VALID`, И ЭТО ЗАМЕР: перед заведением куратор перемерил бой
-- 02.09.2026 — сегментов 168, отрицательных 0, перевёрнутых 0 в обеих парах.
-- Молча менять чужие данные местами запрещено, поэтому порядок был такой:
-- сперва замер, и только потом ограничение.
--
-- Устройство блока — по образцу `call_events_window_rule_check`: снять прежние
-- имена, если они были, и завести своё, если его ещё нет.
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint
                    WHERE conname = 'offer_segment_numbers_check') THEN
        ALTER TABLE real_estate_offer_segments ADD CONSTRAINT offer_segment_numbers_check
            CHECK ((price_min IS NULL OR price_min >= 0)
               AND (price_max IS NULL OR price_max >= 0)
               AND (area_min  IS NULL OR area_min  >= 0)
...
-- ----- Настройки: пять строк экрана (Б9.2, решение владельца 110) ------------
-- Состав закрыт владельцем: семь строк, из них две уже засеяны выше. Часового
-- пояса среди них НЕТ, и это не пропуск: APP_TIMEZONE объявлен явной константой
-- по решению куратора (services/appTime.js:11-13), и паспорт Р8 ссылается на то
-- же решение — «пояс на экран не выносится, и это не упущение».
--
-- ЗАСЕВ ПОДПИСЫВАЕТ СЕБЯ САМ. Триггер журнала ловит вставку, а автора берёт из
-- настроек соединения; без подписи пять новых строк ушли бы в журнал с
-- actor_kind = 'none', и экран настроек — тот самый, где спрашивают «кто это
-- включил», — нарисовал бы их «автор не указан». Ровно эта ошибка стоила
-- проекту семи безымянных записей на выкатке части 5 и корректировки К266
-- (против неё решение владельца 98).
--
-- ПРЕЖНИЕ ДВЕ СТРОКИ НЕ ТРОГАЮТСЯ. Их запись уже произошла без подписи, и
-- переписывать журнал задним числом нельзя.
--
-- ЗНАЧЕНИЯ НЕТ НИ У ОДНОЙ: value остаётся NULL, и строка честно показывает «Не
-- задано». Рубильник засеян значением по обратному доводу — у выключателя
-- «не задано» не бывает.
--
-- МЕТАДАННЫЕ ВСТАВЛЯЮТСЯ СРАЗУ, А НЕ ВТОРЫМ UPDATE. Так первый старт пишет одну
-- запись «строка заведена» вместо двух — «заведён ключ» и следом «у него
-- появились имя, описание и тип». Безусловный UPDATE ниже при этом остаётся: он
-- нужен не первому старту, а всем следующим — описание правится файлом.
DO $$
BEGIN
    ...
    INSERT INTO app_settings
        (key, title, description, value_type, unit, group_key, group_order,
         is_readonly, is_dangerous, default_value)
    VALUES
        ('autodial_line_share',
         'Доля линий под обзвон',
         'Какую часть свободных линий занимает автообзвон. Остальное остаётся операторам.',
         'percent', '%', 'Автообзвон', 20, false, false, NULL),
        ('stale_call_hours',
         'Порог сторожа зависших',
         'Сколько звонок может висеть активным без единого события, прежде чем его закроют исходом «связь потеряна».',
         'number', 'ч', 'Звонки', 30, false, false, '4'),
        ('pbx_webhook_url',
         'Адрес приёма событий от АТС',
         'Куда телефония шлёт события о звонках. Меняется только вместе с настройкой на стороне АТС — иначе события просто перестанут приходить.',
         'text', NULL, 'Звонки', 40, false, false, NULL),
        ('held_lead_release_hours',
         'Лид возвращается в очередь',
         'Сколько оператор может быть вне линии, прежде чем удержанный за ним лид уйдёт обратно в общую очередь.',
         'number', 'ч', 'Операторы', 50, false, false, '2'),
        ('max_open_interval_hours',
         'Потолок открытого интервала',
         'Оператор закрыл вкладку, не выйдя из системы, — интервал состояния висит открытым. Старше этого порога закрывается принудительно.',
         'number', 'ч', 'Операторы', 60, false, false, '12')
    ON CONFLICT (key) DO NOTHING;
END $$;

-- Описание идёт из кода, значение живёт в базе — то же правило, что у двух
-- строк выше: метаданные обновляются безусловно при каждом старте, а `value` в
-- этих UPDATE не упоминается вовсе.
UPDATE app_settings SET
    title = 'Доля линий под обзвон',
    description = 'Какую часть свободных линий занимает автообзвон. Остальное остаётся операторам.',
    value_type = 'percent',
    unit = '%',
    group_key = 'Автообзвон',
    group_order = 20,
    ...
    is_dangerous = false,
    default_value = NULL
 WHERE key = 'autodial_line_share';

UPDATE app_settings SET
    title = 'Порог сторожа зависших',
    description = 'Сколько звонок может висеть активным без единого события, прежде чем его закроют исходом «связь потеряна».',
    value_type = 'number',
    unit = 'ч',
    group_key = 'Звонки',
    group_order = 30,
    ...
    is_dangerous = false,
    default_value = '4'
 WHERE key = 'stale_call_hours';

UPDATE app_settings SET
    title = 'Адрес приёма событий от АТС',
    description = 'Куда телефония шлёт события о звонках. Меняется только вместе с настройкой на стороне АТС — иначе события просто перестанут приходить.',
    value_type = 'text',
    unit = NULL,
    group_key = 'Звонки',
    group_order = 40,
    ...
    is_dangerous = false,
    default_value = NULL
 WHERE key = 'pbx_webhook_url';

UPDATE app_settings SET
    title = 'Лид возвращается в очередь',
    description = 'Сколько оператор может быть вне линии, прежде чем удержанный за ним лид уйдёт обратно в общую очередь.',
    value_type = 'number',
    unit = 'ч',
    group_key = 'Операторы',
    group_order = 50,
    ...
    is_dangerous = false,
    default_value = '2'
 WHERE key = 'held_lead_release_hours';

UPDATE app_settings SET
    title = 'Потолок открытого интервала',
    description = 'Оператор закрыл вкладку, не выйдя из системы, — интервал состояния висит открытым. Старше этого порога закрывается принудительно.',
    value_type = 'number',
    unit = 'ч',
    group_key = 'Операторы',
    group_order = 60,
    ...
    is_dangerous = false,
    default_value = '12'
 WHERE key = 'max_open_interval_hours';

-- ----- Рубильник приёма событий (Е1, решение владельца 145) ------------------
-- ⚠⚠ НАСТРОЙКОЙ НА ЭКРАНЕ, А НЕ ПЕРЕМЕННОЙ ОКРУЖЕНИЯ, и это выбор владельца, а
-- не удобство: он гасит поток САМ и МГНОВЕННО — без правки файла на сервере, без
-- перезапуска службы и без обращения к куратору. На боевом контуре это
-- единственный способ остановиться, не выкатывая.
--
-- ⚠⚠ ЗАСЕВАЕТСЯ ВКЛЮЧЁННЫМ, И ЭТО ОБРАТНО РУБИЛЬНИКУ АВТООБЗВОНА. У того
-- умолчание 'false': свежая выкатка не должна сама никому звонить. Здесь
-- наоборот — выключенный приём означает, что события станции ВЫБРАСЫВАЮТСЯ, а
-- потерянное событие звонка не восстанавливается ничем. Опасное действие у этих
-- двух настроек разное: у автообзвона опасно ДЕЙСТВИЕ, здесь опасно
-- БЕЗДЕЙСТВИЕ.
--
-- ⚠ ПРИ ЭТОМ РАЗБОР ЗНАЧЕНИЯ ОСТАЁТСЯ ОБЩИМ: «включено — это ровно строка
-- 'true'» (`services/appSettings.js`). Заводить второе прочтение того же тумблера
-- нельзя: экран рисует переключатель ПО ЗНАЧЕНИЮ, и «в базе мусор, на экране
-- выключено, а приём идёт» — это ложь на экране. Отсюда состав: строка засеяна
-- 'true', умолчание при ОТСУТСТВИИ строки — тоже включено, а испорченное руками
-- значение честно выключает приём и честно показывает это переключателем.
--
-- ⓘ Опасной помечена намеренно: выключение молча теряет звонки.
DO $$
BEGIN
    PERFORM set_config('crm.audit_actor_kind', 'service', true);
    PERFORM set_config('crm.audit_actor_name', 'Миграция', true);
    ...
    INSERT INTO app_settings
        (key, value, title, description, value_type, unit, group_key, group_order,
         is_readonly, is_dangerous, default_value)
    VALUES
...
-- Описание идёт из кода, значение живёт в базе — то же правило, что у строк
-- выше: `value` в этом UPDATE не упоминается вовсе.
-- ----- Рубильник ИСХОДЯЩИХ звонков из CRM (этап Е2, наряд 04.09.2026) -------
--
-- ⚠⚠ УМОЛЧАНИЕ ОБРАТНОЕ СОСЕДНЕМУ, И ЭТО ГЛАВНОЕ, ЧТО ПРО НЕГО НАДО ЗНАТЬ.
-- У приёма событий (выше) опасно БЕЗДЕЙСТВИЕ: выключенный приём молча теряет
-- звонки, восстановить их нечем, поэтому он засеян включённым. Здесь опасно
-- ДЕЙСТВИЕ: набор звонит живому человеку, и свежая выкатка не имеет права
-- сделать это сама. «Не сработало» замечают и включают; «позвонило само»
-- замечают по последствиям.
--
-- ⓘ Тот же довод, что у рубильника автообзвона, и та же сторона ошибки:
-- разбор значения общий («включено — это ровно строка 'true'»,
-- services/appSettings.js), и всё, что не 'true', честно выключает набор.
--
-- ⚠ ПОРЯДОК 26, А НЕ 20 И НЕ 30. Двадцатку занимает `autodial_line_share` из
-- другой группы, тридцатку — срок хранения записей из этой; экран сортирует
-- по паре «порядок, ключ», и общий номер у разных групп разорвал бы группу
-- пополам (находка дизайн-сессии 03.09.2026). 26 не занято ничем.
DO $$
BEGIN
    PERFORM set_config('crm.audit_actor_kind', 'service', true);
    PERFORM set_config('crm.audit_actor_name', 'Миграция', true);
    ...
    INSERT INTO app_settings
        (key, value, title, description, value_type, unit, group_key, group_order,
         is_readonly, is_dangerous, default_value)
    VALUES
        ('pbx_dial_enabled', 'false',
         'Исходящие звонки из CRM',
         'Пока выключен, кнопка «Набрать» в пульте оператора отвечает отказом и станция не вызывается вовсе. Включение разрешает CRM звонить: набор поднимает трубку у оператора, а затем вызывает клиента.',
         'switch', NULL, 'Звонки', 26, false, true, 'false')
    ON CONFLICT (key) DO NOTHING;
END $$;

-- Описание идёт из кода, значение живёт в базе: `value` в этом UPDATE не
-- упоминается вовсе — иначе выкатка возвращала бы тумблер в исходное.
UPDATE app_settings SET
    title = 'Исходящие звонки из CRM',
    description = 'Пока выключен, кнопка «Набрать» в пульте оператора отвечает отказом и станция не вызывается вовсе. Включение разрешает CRM звонить: набор поднимает трубку у оператора, а затем вызывает клиента.',
    value_type = 'switch',
    unit = NULL,
    group_key = 'Звонки',
    group_order = 26,
    ...
    is_dangerous = true,
    default_value = 'false'
 WHERE key = 'pbx_dial_enabled';

UPDATE app_settings SET
    title = 'Приём событий от АТС',
    description = 'Пока включён, система принимает и записывает сообщения станции о звонках. Выключение останавливает приём немедленно — события за это время будут потеряны безвозвратно, станция их не повторяет.',
    value_type = 'switch',
    unit = NULL,
    group_key = 'Звонки',
    group_order = 25,
...
-- ----- Ключи телефонии: ОТДЕЛЬНАЯ ТАБЛИЦА, И ЭТО ГЛАВНОЕ В НЕЙ ---------------
-- Решение владельца 132: ключи Телфина живут в настройках CRM, показ точками,
-- значение хранится открытым текстом (риск назван владельцу и им принят). Экран
-- — тот же экран настроек. А вот ЛЕЖАТ они не в `app_settings`, и вот почему.
--
-- ⚠⚠ ПРИЧИНА — ЖУРНАЛ, А НЕ ОПРЯТНОСТЬ. Правки настроек пишутся в аудит вместе
-- со значением: на этом стоит подпись «Изменено …» на экране (паспорт Р8,
-- `routes/settings.js` читает журнал по `changes @> '[{"field": "value"}]'`).
-- Уровень правила задаётся ПАРОЙ «таблица + колонка» — `audit_rules` не знает
-- слова `record_id` вовсе. Значит для `app_settings.value` выбор был только
-- такой: либо `full` и App Secret ложится в журнал открытым текстом навсегда,
-- либо `fact`/`skip` — и тогда значений в журнале лишаются ВСЕ СЕМЬ обычных
-- настроек. Ни то, ни другое не годится, поэтому у ключей своя таблица.
--
-- ⚠ ОНА ИСКЛЮЧЕНА ИЗ АУДИТА — пятой строкой в списке ниже по файлу, рядом с
-- `pbx_events`. Это НАЗВАННОЕ отступление от правила, записанного там же:
-- «таблицы настроек в журнал пишутся, изменение настройки объясняет поведение
-- системы». Отступление узкое: объясняет поведение системы ФАКТ наличия ключа,
-- а не сам ключ, и факт виден на экране признаком «задано».
--
-- ⚠ ОПИСАНИЕ СТРОКИ ЛЕЖИТ ЗДЕСЬ ЖЕ, а не в `app_settings`: половина строки в
-- одной таблице и половина в другой означала бы, что экран собирает настройку
-- из двух мест и ломается, когда одна половина отстала. Колонки взяты у
-- `app_settings` теми же именами — не ради красоты, а чтобы сервер отдавал обе
-- породы строк одной формой и экран не различал их вовсе.
--
-- ⚠ `value_type` здесь не колонка: у всех строк этой таблицы он один и тот же —
-- `secret`, и хранить его в каждой строке значило бы разрешить другой.
...
-- Две строки заводятся ПУСТЫМИ: место есть, ключа нет. Пока значение пусто,
-- телефония читает запасной источник — переменные окружения (решение куратора
-- по вопросу 11: настройки побеждают, окружение остаётся запасным).
INSERT INTO pbx_credentials (key, title, description, group_key, group_order)
VALUES
    ('telphin_app_id',
     'App ID Телфина',
     'Идентификатор приложения из кабинета АТС. Заводится там же, где App Secret: регистрация приложения, тип trusted, уровень Call API.',
     'Ключи телефонии', 70),
    ('telphin_app_secret',
     'App Secret Телфина',
     'Пароль приложения. Кто знает — звонит за счёт компании. Значение не показывается обратно ни на экране, ни в ответе сервера: показать его можно было бы только достав из базы, а достать нам незачем.',
     'Ключи телефонии', 80)
ON CONFLICT (key) DO NOTHING;

-- ----- Состояние связи со станцией (этап Е0, решение владельца 144) ---------
-- ⚠⚠ ПОЧЕМУ ЭТО ТАБЛИЦА, А НЕ ПЕРЕМЕННАЯ В ПАМЯТИ. План 7.1: «ничего важного не
-- хранится только в оперативной памяти приложения». Перезапуск службы бывает при
-- каждой выкатке, и после него «связь была вчера в 14:32» обязано остаться
-- известным — иначе полоса отказа после каждой выкатки говорила бы, что связи
-- не было никогда.
--
-- ⚠⚠ ПОЧЕМУ ОТДЕЛЬНАЯ ТАБЛИЦА, А НЕ СТРОКА В СОСЕДНИХ. Оба соседних места
-- проверены и оба не годятся, и не по вкусу, а по коду:
--   · `pbx_credentials` — `routes/settings.js` тянет оттуда ВСЕ строки без
--     отбора и отдаёт каждую на экран настроек полем типа `secret`. Время
--     последнего обмена вылезло бы на экран точками рядом с App Secret;
--   · `app_settings` — правки туда пишутся в журнал изменений ВМЕСТЕ со
--     значением. Время обмена обновляется при каждом удачном запросе и засыпало
--     бы журнал шумом; запрет назван в наряде прямо.
--
-- ⚠ СТРОКА ЗДЕСЬ РОВНО ОДНА, и это стережёт ограничение, а не уговор: состояние
-- связи у системы одно. Без замка вторая строка появилась бы незаметно, и два
-- ответа на один вопрос разошлись бы молча.
--
-- ЧТО РАЗЛИЧАЮТ ТРИ ПОЛЯ (задача 58, решение владельца 144):
--   available = false, last_exchange_at пусто  — ключи есть, связи ещё НЕ БЫЛО;
--   available = false, last_exchange_at задано — связь БЫЛА и пропала;
--   available = true                           — связь есть.
-- Третий случай появился оттого, что ключи вписали раньше, чем построили клиента
-- станции. Без него полоса «Нет связи с телефонией» говорила бы о потере связи,
-- которой никогда не было.
--
-- ⚠ `last_error_code` — КОД, А НЕ ТЕКСТ ОШИБКИ, и это то же правило, что у
-- ключей: в текст ошибки драйвер кладёт параметры запроса, а среди них ключ.
CREATE TABLE IF NOT EXISTS pbx_state (
    id SMALLINT PRIMARY KEY DEFAULT 1 CHECK (id = 1),
    available BOOLEAN NOT NULL DEFAULT FALSE,
    -- Время ПОСЛЕДНЕГО УДАЧНОГО обмена. Неудачный его не трогает: иначе поле
    -- отвечало бы на вопрос «когда мы пробовали», а спрашивают «когда работало».
    last_exchange_at TIMESTAMP,
    last_error_code VARCHAR,
    -- ⚠⚠ ЧАСОВОЙ ЛИМИТ СЧИТАЕТСЯ ПО ЗАГОЛОВКАМ ОТВЕТА, А НЕ ПО ОТКАЗУ: 429
    -- стоит ЧАСА простоя, и узнавать о нём по факту поздно. ⚠ На выдаче токена
    -- (`/oauth/token`) этих заголовков НЕТ ВОВСЕ — замер 03.09.2026; значит
    -- после неё поля остаются прежними, а не обнуляются в ноль.
    rate_limit INTEGER,
...
INSERT INTO pbx_state (id) VALUES (1) ON CONFLICT (id) DO NOTHING;

UPDATE pbx_credentials SET
    title = 'App ID Телфина',
    description = 'Идентификатор приложения из кабинета АТС. Заводится там же, где App Secret: регистрация приложения, тип trusted, уровень Call API.',
    group_key = 'Ключи телефонии',
    group_order = 70
 WHERE key = 'telphin_app_id';

UPDATE pbx_credentials SET
    title = 'App Secret Телфина',
    description = 'Пароль приложения. Кто знает — звонит за счёт компании. Значение не показывается обратно ни на экране, ни в ответе сервера: показать его можно было бы только достав из базы, а достать нам незачем.',
    group_key = 'Ключи телефонии',
    group_order = 80
 WHERE key = 'telphin_app_secret';
...
CREATE TABLE IF NOT EXISTS calls (
    ...
    -- СНИМОК НА МОМЕНТ ЗАВЕРШЕНИЯ, А НЕ ССЫЛКА НА ЛИДА (план 4.3).
    -- funnel_status_id лежит на лиде и перезаписывается при каждом сохранении:
    -- подтягивая его, журнал показал бы у всех звонков к одному человеку
    -- сегодняшний статус. Он начал бы переписывать собственную историю, и
    -- заметить это почти нельзя — цифры-то правильные.
    --
    ...
    -- ⚠⚠ ЭТОТ СНИМОК ОСТАЛСЯ БЕЗ ИСТОЧНИКА, И ЭТО НЕ ОПИСКА, А НЕЗАКРЫТЫЙ ХВОСТ
    -- (К318, замер куратора 02.09.2026 на 08e1498). Заводился он снимком
    -- `leads.notes` по тому же доводу, что и статус двумя строками выше. Б4.1–
    -- Б4.4 (решение владельца 57) увели комментарий в ленту `lead_comments`, а
    -- само `leads.notes` перестало быть правимым и доживает архивом — `notes`
    -- убрано из списка правимых полей в routes/leads.js. Отсюда два вывода, и
    -- второй важнее первого:
    --   · ДОВОД «ПОЛЕ ПЕРЕЗАПИСЫВАЕТСЯ» К КОММЕНТАРИЮ БОЛЬШЕ НЕ ОТНОСИТСЯ.
    --     Лента только растёт: ни правки, ни удаления записи в ней нет вовсе
    --     (services/leadComments.js), переписать себе историю она не может.
    --     Значит причина, по которой снимок вообще завёлся, ОТПАЛА САМА.
    --   · СНИМАТЬ `leads.notes` ТЕПЕРЬ ХУЖЕ, ЧЕМ НЕ СНИМАТЬ НИЧЕГО: у всех
    --     будущих звонков встал бы один и тот же текст, замороженный 30.08.2026.
    --     Это ловушка ровно для того, кто придёт писать сюда по этому описанию.
    --
    -- ПИШУЩЕГО У КОЛОНКИ НЕТ НИ ОДНОГО, и это по-прежнему так.
    -- ⚠⚠ НО ВТОРАЯ ПОЛОВИНА ДОВОДА УСТАРЕЛА, И СНЯТА ПО ФАКТУ (Е2, 04.09.2026).
    -- Здесь стояло «`INSERT INTO calls` во всём проекте отсутствует, таблицу
    -- наполнит телефония». Телефония её уже наполняет: вставок в `calls` в
    -- проекте ДВЕ — разбор события станции (services/pbxEventStore.js) и набор
    -- номера оператором (services/pbxDial.js). Ни одна из них этой колонки не
    -- касается, поэтому вывод про колонку жив, а посылка про пустую таблицу —
    -- нет. ⓘ Тот же довод стоит в routes/leads.js у непривязанной к звонку
    -- записи ленты.
    -- Колонка «Комментарий» в журнале звонков паспортом раздела ТРЕБУЕТСЯ
    -- (пункт 13), и вопрос не в колонке, а в источнике: этот снимок или лента
    -- по `lead_comments.call_id`. РЕШАЕТСЯ ВМЕСТЕ С ТЕЛЕФОНИЕЙ, тогда же, когда
    -- пишется правило привязки записи к звонку. До тех пор колонка пуста
    -- честно, и пустой ей быть правильнее, чем полной вчерашнего текста.
    notes_snapshot TEXT,
...
-- ⚠ ПРАВИЛА ЖУРНАЛА СТОЯТ РЯДОМ С ТАБЛИЦЕЙ, А ПЕРЕНОС — НИЖЕ НАВЕСКИ ТРИГГЕРОВ.
-- Записи о переносе нужны ТРИ вещи разом: сама таблица, правила её журнала и
-- НАВЕШЕННЫЙ на неё триггер. Первые две объявлены здесь, третья появляется
-- только в блоке «Подключение триггера ко всем таблицам» — поэтому сам перенос
-- вынесен туда, под него (К291).
--
-- ⚠ ЗДЕСЬ ЖЕ СНЯТ НЕВЕРНЫЙ ДОВОД, СТОЯВШИЙ НА ЭТОМ МЕСТЕ. Было написано, что
-- правила ниже переноса роняют уровень в `full` и текст комментария уходит в
-- журнал. Опыт по боевому пути этого не подтвердил: записи журнала не было
-- ВОВСЕ — ни с текстом, ни без. Правило «объявляй правила выше записи, которую
-- они описывают» верное, но дефект был не в них, а в отсутствии триггера.
...
-- +7XXXXXXXXXX. Второго формата не заводится.
--
-- ⚠ ОДИН НОМЕР У ДВУХ ОФФЕРОВ ДОПУСТИМ ТОЛЬКО ВНУТРИ ОДНОЙ СЕТИ (К265,
-- решение владельца 118). Прежде здесь стояло «допустим — уникальности здесь
-- нет намеренно», и после правила это стало полуправдой.
--
-- УНИКАЛЬНОСТИ ПО-ПРЕЖНЕМУ НЕТ, И ПРАВИЛА В СХЕМЕ ТОЖЕ. Оно межстрочное:
-- условие связывает ДВЕ строки через сеть их офферов, а CHECK видит одну
-- строку, и частичный уникальный индекс — пару колонок, а не пару строк.
-- Триггер выразил бы, но порядок строк в этом файле — отдельный класс
-- дефектов (К266, К287, К288, К291 за трое суток), и заводить сюда блок ради
-- правила, которое нарушается только через API, значит платить известную
-- цену за неизвестную выгоду.
--
-- ПРАВИЛО ЖИВЁТ В МАРШРУТЕ: `routes/callEvents.js`, `oneNetworkPerPhone` —
-- и повторено на экране (`Calls/js/modules/callsEvents.js`) по общему правилу
-- проекта «всё, что сервер отбивает, экран проверяет сам».
--
...
--
-- ⚠ ПОЧЕМУ ЗДЕСЬ НЕТ `DO + EXECUTE` ПОД `information_schema`, хотя выше по файлу
-- этот приём назван обязательным (`:332-342`) и применён дважды (`:604`, `:771`).
-- Тот урок про ДВА ГОЛЫХ СТЕЙТМЕНТА без замка: первый прогон снимает колонку, а
-- на втором `SELECT` по ней падает — и сервер не поднимается больше никогда.
-- Здесь весь перенос лежит ВНУТРИ `IF NOT EXISTS (замок)`, а PL/pgSQL не
-- разбирает запрос в невзятой ветке: на втором старте `UPDATE` не исполняется и
-- не разбирается. Проверено опытом на живом Postgres, обеими редакциями сразу:
-- блок под замком второй прогон проходит, приём с `EXECUTE` — тоже. Второй
-- сторож здесь был бы замком, который ничего не сторожит (решение 107).
--
...
BEGIN
    FOR v_table IN
        SELECT c.relname AS name
         ...
         WHERE n.nspname = 'public'
           AND c.relkind = 'r'
           -- ⚠ `pbx_credentials` — ПЯТАЯ, и это названное отступление от правила,
           --   записанного выше: «таблицы настроек в журнал пишутся». Ключи АТС
           --   исключены не как настройка, а как СЕКРЕТ: правила аудита задаются
           --   парой «таблица + колонка», по одной строке уровня не поставить, и
           --   значение легло бы в журнал целиком. Факт наличия ключа виден на
           --   экране признаком «задано» — этого системе и довольно.
           -- ⚠ `pbx_state` — ШЕСТАЯ, и отступление здесь ДРУГОЕ, чем у ключей.
           --   Это не секрет и не настройка: строка одна, правит её не человек,
           --   а служба — при каждом удачном обмене со станцией. В журнале
           --   изменений от неё шёл бы поток строк «время обмена: 14:31 → 14:32»,
           --   в которых нет ни автора, ни решения. Журнал объясняет ПОСТУПКИ
           --   людей; тик служебного сердцебиения поступком не является.
           AND c.relname NOT IN ('audit_log', 'audit_batches', 'applied_migrations',
                                 'pbx_events', 'pbx_credentials', 'pbx_state')
...
-- ⚠ БЛОК СТОИТ НИЖЕ НАВЕСКИ ТРИГГЕРОВ, И ЭТО ЕГО ГЛАВНОЕ СВОЙСТВО (К291).
-- `lead_comments` заводится этой же схемой — значит на боевой выкатке таблица
-- НОВАЯ, и триггера журнала на ней в момент переноса ещё нет: он навешивается
-- перебором строкой выше. Пока блок стоял рядом с таблицей, перенос проходил
-- мимо журнала ЦЕЛИКОМ — одна запись в ленте и НОЛЬ записей в журнале.
--
-- КЛАСС, А НЕ СЛУЧАЙ: всякая запись данных в НОВУЮ таблицу, поставленная выше
-- навески, теряет журнал. В этом файле так уже случалось четырежды, и все четыре
-- заперты `applied_migrations` — их записей в журнале не появится уже никогда:
--   a89a5f7  app_settings                          (журнал рождался тем же файлом)
--   70e3b06  phone_fix_reasons
--   847b645  call_events, call_recall_rules
--   98e9e31  lead_funnel_stages
--
-- ⚠ СЛУЧАЕВ НЕ ЧЕТЫРЕ, А СЕМЬ, И ЭТО ЗАМЕР НА ЧИСТОЙ БАЗЕ (К287, 02.09.2026).
-- Схема на пустой базе наполняет девять таблиц. Журнал получают ДВЕ, и только
-- частью строк: `audit_rules` 9 записей из 74 и `audit_ref_map` 5 из 51 — ровно
-- те блоки, что стоят ниже навески. Остальные семь идут мимо целиком:
-- `app_settings` 7 строк, `param_lists` 72, `lead_funnel_statuses` 51,
-- `lead_funnel_stages` 8, `call_events` 3, `call_recall_rules` 4,
-- `phone_fix_reasons` 6. Журнал достаётся не по решению, а по тому, куда попал
-- блок, — и это единственная настоящая беда здесь.
--
-- ⚠⚠ НО ПЕРЕНОС ЗАСЕВА ВНИЗ ЭТОГО НЕ ЧИНИТ. Проверено опытом, а не рассуждением:
-- второй прогон схемы на базе, где строки уже есть, дал НОЛЬ новых записей —
-- запертые блоки не выполняются вовсе, незапертые ничего не вставляют из-за
-- `ON CONFLICT DO NOTHING`. Значит боевая установка не получит их НИКОГДА, а
-- новая получила бы двести. Перенос РАЗВЁЛ БЫ установки, а не сблизил.
-- (Проба прибора: нарочная правка описания настройки дала ровно одну запись
-- `update stale_call_hours · service / Миграция`, возврат — вторую. Ноль выше
-- настоящий, а не слепой замер.)
--
-- ОТСЮДА ПРАВИЛО, И ОНО ОТЛИЧАЕТ ЭТОТ БЛОК ОТ ЗАСЕВА:
--   · ЗАСЕВ ПОСТАВКИ — справочники, одинаковые на всех установках и целиком
--     известные из этого файла. Журнала не требует: журнал объясняет историю
--     установки, а не содержимое поставки. Место в файле любое.
--   · МИГРАЦИЯ НАКОПЛЕННОГО — трогает данные, которые ввели люди, и одна и та же
--     схема даёт на двух установках разный итог. Журнал обязателен, блок обязан
--     стоять НИЖЕ навески. Ровно этот случай.
--

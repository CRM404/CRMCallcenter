...
export const registry = [
    ...
    // ДЕВЯТЫЙ РАЗДЕЛ, третий без legacyUrl: старой страницы у настроек не было
    // никогда — их правили выкаткой новой версии, ради чего экран и заведён.
    //
    // ПАСПОРТ Р8 ОТВОДИЛ ИМ СЛУЖЕБНУЮ ПЛИТКУ, А НЕ СТРОКУ РЕЕСТРА, и довод у
    // него был: «встанут в ряд с „Лидами" и будут читаться как ещё один
    // справочник». Решение куратора от 30.08.2026 обратное: служебных плиток с
    // модулем в оболочке нет НИ ОДНОЙ (`shell/desktop.js:23` — одна ссылка в
    // новую вкладку), и заводить весь механизм ради одного раздела дороже, чем
    // девятая строка здесь. Расхождение с паспортом записано в протокол.
    //
    // ПОДПИСЬ ПЛИТКИ — СВОЯ, короче паспортной (правило И171): паспортная
    // фраза «поведение системы: тумблеры, окна времени, пороги» стоит подписью
    // раздела внутри панели, в `settings-section.html`.
    //
    // СКЕЛЕТ — 'form'. Паспорт просит силуэт строки настройки, а форм в слое
    // две; заводить третью ради одного раздела — правка слоя, и она дороже
    // приблизительного силуэта (ответ куратора 28).
    { key: 'settings',   title: 'Настройки', subtitle: 'тумблеры, окна и пороги', icon: 'gear', module: '/js/modules/settingsApp.js', template: '/settings-section.html', styles: '/css/settings-light.css', skeleton: 'form' }
];

...
export const CONFIGURABLE_COLUMNS = [
    ...
    // ⚠ ДВЕ КОЛОНКИ СВЕРХ «Цена и площадь по сегментам», и они про ДРУГОЕ.
    // Та показывает, СКОЛЬКО у оффера сегментов, и прячет перечень в подсказку;
    // эти две отвечают на вопрос «в какие деньги оффер укладывается вообще» —
    // самая низкая нижняя граница и самая высокая верхняя ПО ВСЕМ сегментам.
    // Держать это одной колонкой нельзя: по своду не отсортируешь, а вопрос
    // «от скольки» задают чаще, чем «сколько сегментов».
    { key: 'segPriceMin', label: 'Цена сегмента, ₽ от', num: true },
    { key: 'segPriceMax', label: 'Цена сегмента, ₽ до', num: true },

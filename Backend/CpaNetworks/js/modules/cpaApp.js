...
/* Слова свои, правило — одно на проект: обёртки над общим помощником
   (`/plural.js`, К308).

   ⚠ ЭТА ШАПКА ОСИРОТЕЛА ПРИ К308 И ПОДОБРАНА К320. Она стояла над файловой
   `function plural(...)`, которую К308 сняла, вписав вместо неё ввоз общего, —
   а сама осталась и была отбита от этих трёх строк пустой строкой. Пояснение,
   отбитое пустой строкой, не описывает ничего: читатель видит заголовок без
   предмета и достраивает предмет сам.

   💡 Промах того же захода, что и восьмая копия ниже, и той же природы: снимая
   объявление, надо снимать и его довод — либо переписывать под то, что
   осталось. */
 ...
 *   вложенный список (сегменты) — ЧИСЛОМ со словом, а полный перечень уезжает
 *                    в подсказку `title`. Пять сегментов в ячейку не помещаются,
 *                    а строка таблицы не место для разворота: он есть в карточке
 *                    оффера;
 *   география      — ГОРОДОМ, а не числом (К314). Счёт со словом законен там,
 *                    где различает записи; «1 адрес» стоял у сорока двух офферов
 *                    из сорока четырёх и не различал ни одной строки;
 ...
 *
 * ⚠ КОЛОНКА ОСТАЛАСЬ ОДНА. География ушла на свой вид (К314): у неё счёт не
 * различал записей — сорок два оффера из сорока четырёх несли по одной строке.
 * У сегментов на сорок пять офферов приходится сто шестьдесят восемь, и «3
 * сегмента» против «12 сегментов» — разные строки таблицы. Правило общее:
 * число со словом законно там, где оно различает записи.
 *
 * ⚠⚠ ЗДЕСЬ ЖИЛА ВОСЬМАЯ КОПИЯ ПРАВИЛА СКЛОНЕНИЯ (К320). К308 свела семь копий
 * к общему помощнику и прошла мимо этой — в том самом файле, который сама
 * правила: она сняла отсюда объявление `function plural(...)` и вписала ввоз,
 * но не тронула ВТОРУЮ реализацию, спрятанную выражением внутри этой функции.
 *
 * 💡 ПОЧЕМУ НЕ НАШЛИ: сведение искали по объявлениям и нашли все объявления.
 * Копия объявлением не была — она была тремя тернарными выражениями над `n`, и
 * по имени её найти было нельзя. Ищется такое по АРИФМЕТИКЕ, а не по имени:
 * `% 100 >= 11`, `% 10 === 1`.
 *
 * ⚠ НА ЭКРАНЕ КОПИЯ НЕ ВРАЛА: на всех десяти пограничных числах она давала те
 * же слова, что общий помощник (1 · 2 · 4 · 5 · 11 · 12 · 14 · 21 · 101 · 111).
 * Дефект был не в ответе, а в том, что одно правило лежало в двух местах и
 * первая же правка одного из них развела бы их молча.
 */
function listCell(list, one, few, many, describe) {
    ...
    return `<span class="cpa-cell-many" title="${escapeHtml(title)}">${n} ${plural(n, one, few, many)}</span>`;
}
...
/**
 * Ячейка географии: ГОРОД, а не число (К314, макет `3858ab30` ред. 1, решение
 * владельца через куратора; приёмка куратора 02.09.2026).
 *
 * ГОРОД, А НЕ САМЫЙ МЕЛКИЙ ЗАПОЛНЕННЫЙ УРОВЕНЬ. Иначе колонка отвечала бы в
 * каждой строке на разный вопрос — где регионом, где районом, — и сравнивать
 * строки стало бы нечем. КОЛОНКА ОБЯЗАНА ОТВЕЧАТЬ НА ОДИН И ТОТ ЖЕ ВОПРОС
 * ВСЕГДА. Города нет — регион: он заполнен всегда.
 *
 * ⚠ СОВПАДЕНИЕ РЕГИОНА И ГОРОДА СХЛОПЫВАЕТСЯ. В городах федерального значения
 * они равны по устройству справочника, и «Москва, Москва» — это вид записи, а
 * не сведения: показывать его значит выдавать устройство базы за содержание.
 * Схлопывание здесь ничего не склеивает — в ячейку и так идёт один уровень;
 * оно решает, ЕСТЬ ЛИ ЧТО ПРЯТАТЬ.
 *
 * ⚠⚠ ПУНКТИР ОБЕЩАЕТ ПОДСКАЗКУ, значит ставится только там, где в подсказке
 * есть то, чего нет в ячейке. Считается по РАЗЛИЧНЫМ уровням: «Москва, Москва»
 * — один уровень, скрытого нет, ни пунктира, ни подсказки. «Московская
 * область, Химки» — два, регион скрыт, пунктир есть. Несколько адресов —
 * пунктир есть всегда: в ячейку попал только первый. ПУНКТИР, КОТОРЫЙ НИЧЕГО
 * НЕ ОБЕЩАЕТ, ОБЕСЦЕНИВАЕТ ПУНКТИР ТАМ, ГДЕ ОБЕЩАНИЕ НАСТОЯЩЕЕ (правило
 * куратора, распространено им на весь проект).
 *
 * ПОДСКАЗКА НЕ ТРОНУТА: те же уровни через запятую, адреса через точку с
 * запятой — `geoTitle` выше. Схлопывание её не касается: она показывает
 * запись как есть, и в этом её польза.
 *
 * ⚠ ГРАНИЦА, КОТОРОЙ МАКЕТ НЕ ОПИСЫВАЕТ: адрес, у которого пусты И город, И
 * регион. Схема их не требует (`schema.sql:543-552` — все четыре `VARCHAR` без
 * `NOT NULL`), но в бою регион заполнен у ВСЕХ девяноста строк (замер куратора
 * 02.09.2026), то есть сегодня случай не наступает. Взят первый адрес, который
 * на вопрос колонки отвечает; не отвечает ни один — прочерк, по тому же правилу,
 * по которому прочерк стоит у пустой географии.
 */
function geoCell(list) {
    const rows = Array.isArray(list) ? list : [];
    if (!rows.length) return DASH;

    const label = (g) => String(g.city || g.region || '').trim();
    const head = rows.map(label).find(Boolean) || '';
    if (!head) return DASH;

    const text = rows.length > 1 ? `${head} +${rows.length - 1}` : head;

    // Скрытое: у одного адреса — различные уровни после схлопывания, у
    // нескольких — всегда, потому что в ячейку попал только первый.
    const levels = rows.length === 1
        ? new Set([rows[0].region, rows[0].city, rows[0].district, rows[0].locality]
            .map((v) => String(v || '').trim()).filter(Boolean))
        : null;
    const hidden = rows.length > 1 || levels.size > 1;
    if (!hidden) return `<span>${escapeHtml(text)}</span>`;

    const title = rows.map(geoTitle).filter(Boolean).join('; ');
    return `<span class="cpa-cell-many" title="${escapeHtml(title)}">${escapeHtml(text)}</span>`;
}
...
// ⚑ ЗАГОЛОВОК ГРУППЫ — УЗЕЛ СЛОЯ (задача 65, решения владельца 124 и 148).
// Значок 28×28 и подпись-пояснение УБРАНЫ по прямому слову владельца 148:
// «как у остальных сделать» — а у «Лидов» и оператора ни того, ни другого нет.
//
// ⚠ ДОВОДЫ `icon` И `sub` ОСТАВЛЕНЫ В ПОДПИСИ ФУНКЦИИ НАМЕРЕННО, И ЭТО НЕ
// НЕДОДЕЛКА. Их значения приходят из четырёх мест вызова и несут смысл, который
// пригодится, если владелец захочет вернуть подпись: `sub` — единственное место
// в проекте, где записано, ЧЕМ группа отличается от соседней. Стереть их из
// вызовов значило бы стереть и это знание. Функция их просто не рисует.
// ⓘ Если решение окажется окончательным — снимать доводы вместе с текстами,
// одним заходом и с записью в реестр, а не молча.
function sectionHead(icon, title, sub) {
    return `
        <div class="ui-form-head cpa-form-sec">
            <h3 class="ui-form-head__title">${escapeHtml(title)}</h3>
        </div>`;
}

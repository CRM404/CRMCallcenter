...
// К257: было 164 знака и три строки под полем, стало 108 и две. Перечень
// режимов ушёл — у оператора график фиксированный, — но список НЕ ЗАКРЫТ:
// «например» вместо «5/2, 3/3, 2/2, 4/2» стоит намеренно. Поле принимает любую
// пару N/M от 1 до 31, разбор один и тот же на форме и на сервере
// (`employeesScheduleTime.js`, `routes/employees.js`), и закрытый список отбил
// бы уже заведённые графики, которых мы не видели.
const WORK_SCHEDULE_HINT = 'Рабочих дней подряд и выходных подряд, например 5/2. Справочная запись: выходные месяца отсюда не считаются.';
    ...
    function buildHistoryPane() {
        ...
        pane.innerHTML = `
            <div class="hi-section">
                ...
                <div class="ui-empty" data-role="hi-empty" hidden>

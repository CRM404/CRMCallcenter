...
/**
 * УКАЗАНИЕ ИЗ АДРЕСА: `#/employees?record=42` — открыть карточку сотрудника.
 *
 * Ставит такие ссылки журнал изменений: колонка «Запись» ведёт в карточку, и
 * переход в неё — смысл колонки. Разделов с карточкой два, «Лиды» и
 * «Сотрудники» (ответ куратора И209); «Лиды» научились этому частью 7Б, а
 * «Сотрудники» остались без обработчика — и оболочка молча выбрасывала
 * указание, как и написано в её собственном комментарии. Отказ был тихий:
 * адрес менялся, карточка не открывалась, ошибок ноль (К202).
 *
 * СОТРУДНИК БЕРЁТСЯ С СЕРВЕРА, А НЕ ИЗ ЗАГРУЖЕННОГО СПИСКА, — этим уже занят
 * `openById`: он же дочитывает документы карточки. Своего второго пути к записи
 * раздел не заводит.
 *
 * ОТКАЗ ЗДЕСЬ НЕ РОНЯЕТ РАЗДЕЛ: сотрудника могли уволить и удалить, пока ссылку
 * несли. Тогда человек остаётся в открытом списке — это лучше, чем пустая
 * панель.
 */
export async function applyParams(params) {
    const id = Number(params && params.record);
    if (!Number.isInteger(id) || id <= 0) return;
    // Своей проверки «панель ещё жива» здесь нет намеренно: она стоит внутри
    // `openById`, и вторая, снаружи, разошлась бы с первой при первой же правке.
    if (card) await card.openById(id);
}

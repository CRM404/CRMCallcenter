...
// оговорка и пропускается, а плашку читают за секунду до набора.
//
// ⚠ ТЕПЕРЬ ЭТО УЗЛЫ СЛОЯ — .ui-note__title и .ui-note__text (К310). Прежде
// здесь стояло «узла .ui-note__title здесь нет — страница оператора живёт вне
// слоя», и заголовок выделялся полужирным вручную. Слой подключён к странице
// коммитом c193a82, и с тех пор это было неправдой.
//
// Это второй такой случай подряд (первый — К173 в части 1Б, тело окна одним
// абзацем): «что случилось» и «что делать» держим двумя частями всегда.
const PHONE_NOTES = {
    hopeless: {
...
function phoneNote(lead) {
    ...
    // ⚠ ОБА ВИДА — --warn, и безнадёжный тоже. Слой различает виды по
    // обратимости ДЕЙСТВИЯ: --danger это «уже пошло не так или пойдёт
    // необратимо». Здесь действия нет вовсе — есть состояние данных, и оно
    // случилось до того, как оператор открыл карточку. Красная плашка сказала

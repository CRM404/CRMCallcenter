...
export function createOfferTabPicker({
    ...
    //
    // ⚠ ЧИСЛО СТОИТ В ТЕКСТЕ, А НЕ В ЗАГОЛОВКЕ, и склонения в нём нет.
    // Форма взята с написанного прецедента, а не сочинена:
    // `Sources/js/modules/sourcesSection.js:989-993` — «Удалить источники?» и
    // «Будет удалено источников: N». Так же устроено массовое удаление лидов в
    // паспорте раздела. Подпись кнопки повторяет глагол вопроса — правило
    // паспорта «Лидов».
    async function clearAll() {
        ...
        const ok = await confirm({
            ...
            message: `Будет убрано офферов: ${removed}. Вернуть их можно только по одному.`,

...
const RICH_CELLS = {
    ...
    funnelStatus: (l) => {
        ...
        // ⚠ КРАСИТ `awaitsManager`, А НЕ `isSystem` — это К260. Системных
        // статусов два, а красный по решению владельца 106 один: тот, по
        // которому лид ждёт решения руководителя. Первая редакция К245 красила
        // оба, и «Не ответил после N перезвонов» становился красным без
        // основания.
        const tone = l.statusAwaitsManager ? 'ui-pill--bad' : 'ui-pill--mute';
...
/**
 * УКАЗАНИЕ ИЗ АДРЕСА: `#/leads?record=1042` — открыть карточку этой записи.
 *
 * Ставит такие ссылки раздел «Звонки»: телефон лида в журнале ведёт в карточку,
 * и переход в неё — смысл колонки (ответ куратора И169). Приводить человека в
 * список, где нужную строку ещё надо найти, значит не выполнить обещание
 * ссылки.
 *
 * ЛИД БЕРЁТСЯ С СЕРВЕРА, А НЕ ИЗ ЗАГРУЖЕННОГО СПИСКА. Список раздела — это
 * первая страница текущего отбора; лид из вчерашнего звонка в неё не попадает
 * почти никогда, и карточка не открылась бы ровно в том случае, ради которого
 * ссылку и ставили.
 *
 * ОТКАЗ ЗДЕСЬ НЕ РОНЯЕТ РАЗДЕЛ: лида могли удалить или увести в архив, пока
 * ссылку несли. Тогда человек остаётся в открытом списке и видит объяснение —
 * это лучше, чем пустая панель.
 */
export async function applyParams(params) {
    const id = Number(params && params.record);
    if (!Number.isInteger(id) || id <= 0) return;
    const my = generation;
    try {
        const lead = await storage.fetchLeadById(id);
        if (my !== generation || !lead) return;
        await leadModal.open(lead);
    } catch (err) {
        if (my !== generation || isAbort(err)) return;
        shell.toast(err.message, 'error');
    }
}

...
const FIELD_COLUMNS = [
    ...
    // ⓘ ЗДЕСЬ СТОЯЛО ПРЕДУПРЕЖДЕНИЕ, И ОНО УСТАРЕЛО ДВАЖДЫ (снято задачей 43).
    // Оно требовало: «любое поле здесь обязано быть и в rowToEmployee ниже —
    // massActions.js отправляет обратно ВЕСЬ объект, а PUT перезаписывает все
    // колонки из этого списка; забытое поле = молчаливое обнуление при массовом
    // переводе в неактивные».
    //   1. Файла `massActions.js` больше нет — он слит в `employeesTable.js`, и
    //      массовые действия давно ходят своим маршрутом `POST /bulk-archive`,
    //      а не через этот PUT.
    //   2. Само обнуление снято задачей 43: колонка, которой нет в теле, не
    //      трогается, и забытое поле больше не стирает значение.
    // Требование «поле должно быть в rowToEmployee» остаётся верным по другой
    // причине — иначе форма не покажет то, что хранит база.
...
router.put('/:id', async (req, res) => {
    ...
    try {
        ...
        // ⚠⚠ СТАТУС, КОТОРЫЙ НЕ ПРИСЛАЛИ, НЕ МЕНЯЕТСЯ (задача 43). Прежде здесь
        // стоял голый `normalizeValue('status', req.body.status)`, а он на
        // отсутствующий ключ отдаёт не null, а 'active' — то есть тело без
        // ключа `status` означало «сделать активным», и архивная карточка
        // воскресала бы молча. Это опаснее обнуления: остальные колонки от
        // неполного тела становились пустыми, а эта — ЗНАЧАЩЕЙ.
        const statusSent = Object.prototype.hasOwnProperty.call(req.body, 'status');
        const willBeArchived = statusSent
            ? normalizeValue('status', req.body.status) === 'inactive'
            : wasArchived;
        ...
        // ⚠⚠ КОЛОНКА, КОТОРУЮ НЕ ПРИСЛАЛИ, НЕ ТРОГАЕТСЯ ВОВСЕ (задача 43).
        // Было: список строился из ВСЕХ колонок разом, и отсутствующий ключ
        // давал `normalizeValue(key, undefined)`, то есть null. Тело без ключа
        // стирало колонку молча.
        //
        // ⓘ Сегодня это ничем не проявлялось, и проверено это счётом, а не на
        // глаз: PUT зовут ДВА места, и оба шлют объект целиком — форма карточки
        // (28 ключей, сверены поимённо с этим списком: ни одного лишнего, ни
        // одного недостающего) и возврат из архива (`{ ...card, status,
        // pbxExtension }`). Массовые действия сюда не ходят вовсе — у них свой
        // маршрут `POST /bulk-archive`. То есть защита держалась на СОГЛАШЕНИИ
        // «клиент шлёт всё», а не на коде, и любой другой клиент с частичным
        // телом стёр бы остальное.
        //
        // Приём тот же, что у GUARDED_COLUMNS ниже: различение «ключа нет» и
        // «ключ прислан пустым». Там оно стояло с самого начала, здесь его не
        // было — при том что список тут в четырнадцать раз длиннее.
        //
        // ⓘ Пустым `setClauses` быть не может: четыре ключа из этого списка
        // обязательны (REQUIRED_FIELDS) и отсутствие любого отбивается выше.
        const values = [];
        const setClauses = [];
        FIELD_COLUMNS.forEach(([key, col]) => {
            if (!Object.prototype.hasOwnProperty.call(req.body, key)) return;
            values.push(normalizeValue(key, req.body[key]));
        ...
        // не шлёт, а значит и не может ими распоряжаться. ⓘ Прежде довод здесь
        // был другим — «PUT перезаписывает весь список, и первое же массовое
        // „перевести в неактивные" стёрло бы выданные ключи всем разом». Задача
        // 43 эту мину сняла: список больше не переписывается целиком. Вывод от
        // этого не изменился, изменилась его причина, и она названа заново.
        //
        // ⚠ ЗДЕСЬ БЕРЁТСЯ ФАКТИЧЕСКИЙ БУДУЩИЙ СТАТУС, А НЕ ПРИСЛАННЫЙ КЛЮЧ
        // (задача 43): тело без ключа `status` прежде давало 'active', и отзыв
        // ключа не срабатывал бы там, где карточка на самом деле уходит в архив.
        values.push(willBeArchived ? 'inactive' : 'active');
        ...
        // ВИД АРХИВА И ДАТА ЗАМОРОЗКИ. Считаются здесь, а не в FIELD_COLUMNS,
        // потому что это НЕ ПОЛЕ ФОРМЫ: вид архива и дата заморозки выводятся из
        // перехода, а не вводятся человеком.
        //
        // ⓘ Прежний довод был про мину — «PUT переписывает ВЕСЬ список, и
        // сохранение карточки старым интерфейсом стёрло бы вид архива всем,
        // молча». Задача 43 её сняла: колонка, которой нет в теле, теперь не
        // трогается. Довод переписан, а не оставлен: он объяснял бы устройство
        // опасностью, которой больше нет.

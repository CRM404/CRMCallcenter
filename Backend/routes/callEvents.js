 ...
 * окне множественного выбора, и две дыры оставались открытыми: ДВА РАЗДЕЛЬНЫХ
 * ВЫБОРА с одним номером — второй о первом не знает вовсе; и ПРАВКА НОМЕРА в
 * уже стоящей строке — поле правится всегда.
 *
 * ⚠⚠ ТОГО ЗАПРЕТА БОЛЬШЕ НЕТ (К309, решение владельца 134): он был ни необходим,
 * ни достаточен, и окно пускает теперь офферы любых сетей. Эта проверка и её
 * экранная половина остались единственными.
 *
...
router.get('/directories', async (req, res) => {
    try {
        const [statuses, scripts, offers, employees] = await Promise.all([
            ...
            // СЧИТАЕТ сети выбора: строка счёта называет их число и говорит,
            // что одним номером их не покрыть (К309; прежде первый отмеченный
            // оффер выключал офферы чужих сетей — этого больше нет). Считать по
            // имени нельзя — `cpa_networks` объявлена без `UNIQUE` на `name`
            // (`schema.sql:401-409`), и две одноимённые сети слились бы в одну
            // молча, занизив счёт. Имя тоже едет: его показывают подстрокой у
            // каждого оффера и называют им сеть, когда она в выборе одна.
            pool.query(

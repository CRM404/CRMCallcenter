...
const T = {
    ...
    // выбирается по `default_value`, а он пуст у `autodial_line_share` и
    // `pbx_webhook_url`. Умолчания у них нет ни в базе, ни в коде — значит и
    // читать его неоткуда, и настройка просто не действует.
    // Имя тоже врало: `FromCode` обещало умолчание в коде, которого нет.
    noDefault: 'Не задано — умолчания нет, настройка не действует',
...
function createSection(container, ctx) {
    ...
    function renderField(row, edited, error) {
        ...
        if (error) {
            ...
            // есть, и «ничего», если его нет. Умолчание называется ЧИСЛОМ —
            // ради этого колонка `default_value` и заведена.
            // ⚠ ФОРМ ПОДПИСИ ТРИ, И ТРЕТЬЯ — ЭТО К283 (паспорт Р8, ред. 3):
            //   · значение задано            — подписи нет вовсе;
            //   · задано не оно, а умолчание — «работает умолчание 4 ч», числом;
            //   · умолчания нет тоже         — «умолчания нет, настройка не действует».
            // Человек не должен слышать про умолчание там, где его нет.

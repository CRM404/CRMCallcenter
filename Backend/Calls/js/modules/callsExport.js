...
export async function buildWorkbook(rows, filters) {
    ...
    const data = rows
        ...
        .map((r) => ({
            ...
            // ОТДЕЛЬНЫЙ СТОЛБЕЦ, А НЕ КОЛОНКА НА ЭКРАНЕ (паспорт Р1 ред. 8): в
            // таблице колонок двенадцать и тринадцатой не будет, а в файле
            // разворачивать нечего — офферы едут строкой, все, через запятую.
            'Оффер перевода': r.transferOffers || '',
            'Комментарий': r.notes || '',
            // Объясняет, почему у долгого разговора короткий комментарий, и
            // снимает вопрос к оператору.
            'Карточка': r.partiallyFilled ? 'заполнена частично' : ''
        }));

...
/** Четыре события со своими перечнями. Вкладка «События». */
export function fetchEvents(api) {
    return api.get('/call-events');
}

/**
 * Справочники четырёх окон одним ответом: статусы, скрипты, офферы, сотрудники.
 *
...
// Четыре сохранения по числу окон. Каждое возвращает события ЦЕЛИКОМ — экран
...
export function saveTransferWait(api, waitSeconds) {
    return api.put('/call-events/transfer-wait', { waitSeconds });
}

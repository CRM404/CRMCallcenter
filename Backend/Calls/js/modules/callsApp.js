...
function createInstance(container, ctx) {
    ...
    function doneRow(row) {
        ...
        // Статус ставил человек, и цветом он не оценивается: красить «Недозвон»
        // в красное значит оценивать работу оператора, а раздел — не табель.
        if (row.funnelStatus) status.appendChild(span('ui-pill ui-pill--mute', row.funnelStatus));

...
// СРОК ХРАНЕНИЯ ЗАПИСИ У ОПЕРАТОРА СВЯЗИ — ГОД (решение владельца 127 от
// 02.09.2026, паспорт раздела, состояние 13а). Срок НЕ НАШ: записи держит
// Телфин, мы только знаем, когда спрашивать уже нечего.
//
// ГОД СЧИТАЕТСЯ КАЛЕНДАРНЫЙ, а не 365 суток: решение названо словом «год», и
// в високосный год 365 суток дали бы ответ на сутки раньше срока — то есть
// кнопка гасла бы, когда запись ещё есть.
//
// ⚠ ЗАШИТО, И ЭТО НАЗВАНО, А НЕ СПРЯТАНО. Паспорт настроек (Р8) относит сроки
// к значениям, которые в код зашивать нельзя. Здесь срок зашит потому, что
// настройки такой нет ни в схеме, ни в бою (семь ключей, среди них срока
// хранения записей нет), а заводить её самовольно — выдумывать за владельца
// и за дизайн-сессию. Перенос в настройку — отдельная работа со своим
// номером; пока она не сделана, единственное место срока — эта строка.
const RECORD_KEPT_YEARS = 1;

// Текст неактивной кнопки — ЗНАК В ЗНАК тот же, что у вкладки «Звонки» в
// карточке лида (паспорт Р9). Две копии одного текста расходятся в первый же
// день правки; вторая копия появится, когда Р9 будет построена, и брать её
// придётся отсюда.
const RECORD_GONE_TEXT = 'Запись у оператора связи не сохранилась';
...
function createInstance(container, ctx) {
    ...
    function doneRow(row) {
        ...
        // ТРИ СЛУЧАЯ, А НЕ ДВА (К319, паспорт раздела, состояния 13 и 13а).
        //
        // КНОПКИ НЕТ ВОВСЕ, ПОКА НЕТ ЗАПИСИ (ответ куратора И178). Неактивная
        // кнопка обещает то, чего нет, тост на неё — тем более.
        //
        // ⚠⚠ И178 НЕ ЗАПРЕЩАЕТ КНОПКУ 13а, А ДЕЛИТ СЛУЧАИ, и спутать их легко.
        // Там записи НЕ БЫЛО НИКОГДА — неактивная кнопка обещала бы небывшее.
        // Здесь запись БЫЛА и её больше нет: кнопка вместе со своим текстом
        // говорит ровно это. Прочерк на её месте соврал бы, что разговор не
        // записывался, а он записывался.
        if (!row.hasRecord) rec.appendChild(dash());
        else if (recordExpired(row.startedAt)) rec.appendChild(recordGoneButton());
        else rec.appendChild(playButton(row.id));
        tr.appendChild(rec);
    ...
    /**
     * Истёк ли срок хранения записи у оператора связи (состояние 13а).
     *
     * ⚠ НЕИЗВЕСТНАЯ ДАТА — НЕ «ИСТЁК». Пустое или неразборчивое время начала
     * даёт `false`: сказать «записи больше нет», не зная возраста звонка,
     * значит спрятать рабочую кнопку по догадке. Ошибиться в эту сторону
     * дороже — человек не услышит запись, которая есть.
     */
    function recordExpired(startedAt) {
        if (!startedAt) return false;
        const started = new Date(startedAt);
        if (Number.isNaN(started.getTime())) return false;
        const edge = new Date();
        edge.setFullYear(edge.getFullYear() - RECORD_KEPT_YEARS);
        return started < edge;
    }

    /**
     * Неактивная кнопка состояния 13а.
     *
     * ЗНАЧОК ТОТ ЖЕ, ЧТО У РАБОЧЕЙ КНОПКИ, и это не небрежность: колонка
     * обязана читаться одним взглядом сверху вниз, а место записи узнаётся по
     * значку. Разный значок заставил бы разбирать каждую строку заново.
     *
     * ОБЪЯСНЕНИЕ В ДВУХ МЕСТАХ РАЗОМ — `title` для мыши, `aria-label` для
     * чтения с экрана (паспорт раздела). Одного `title` мало: до него ещё
     * надо додуматься навести.
     */
    function recordGoneButton() {
        const btn = document.createElement('button');
        btn.type = 'button';
        btn.className = 'ui-btn ui-btn--ghost ui-btn--icon';
        btn.disabled = true;
        btn.title = RECORD_GONE_TEXT;
        btn.setAttribute('aria-label', RECORD_GONE_TEXT);

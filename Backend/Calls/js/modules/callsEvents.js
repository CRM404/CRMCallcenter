...
export function createEventsTab({ pane, api, scope }) {
    ...
    function openTransferWindow() {
        ...
        function offerRow(row, index) {
            ...
            // где есть поиск. Оставь список в строке — и у одного и того же
            // выбора стало бы два входа. Чтобы сменить оффер, строку убирают и
            // заводят заново.
            //
            // ⚠ ПРО «ОДНУ СЕТЬ» ЗДЕСЬ БОЛЬШЕ НЕ ГОВОРИТСЯ, и это правка по
            // существу, а не по слогу: до К309 довод опирался на то, что окно
            // стережёт правило, а строка — нет. Теперь правило живёт у номера и
            // стережёт ОБА входа сразу, включая ручную правку номера в этой
            // самой строке (К265).
            //
        ...
        //
        // ⚠ ОКНО БОЛЬШЕ НЕ ТРЕБУЕТ ОДНОЙ СЕТИ — К309, решение владельца 134.
        // Прежде первый отмеченный оффер выключал офферы чужих сетей, а причина
        // стояла в самой строке. После К265 это ограничение стало НИ
        // НЕОБХОДИМЫМ, НИ ДОСТАТОЧНЫМ: двух дыр — два раздельных захода с одним
        // номером и правка номера в уже готовой строке — оно не закрывало, а
        // законный случай запрещало (взять офферы разных сетей и развести им
        // номера в перечне). Правило переехало в ДАННЫЕ и принадлежит теперь
        // НОМЕРУ: «у одного номера может быть только одна сеть».
        //
        // ГДЕ СТОИТ ОБЪЯСНЕНИЕ. Подсказка обязана стоять там, где правило
        // действует. Ограничение стояло на выборе — и объяснение стояло под
        // поиском; теперь правило принадлежит номеру, и объяснение переехало
        // под «Номер для перевода». Под поиском не осталось ничего: сообщать об
        // отсутствии запрета незачем, а прежний текст первой же половиной
        // обещал бы то, чего окно больше не делает.
        //
        // СЕТИ СЧИТАЮТСЯ ПО НОМЕРУ, А НЕ ПО ИМЕНИ: `cpa_networks` объявлена
        // без `UNIQUE` на `name` (`schema.sql:401-409`), и две одноимённые сети
        // слились бы в одну молча — счёт занизился бы, и строка счёта
        // промолчала бы ровно там, где обязана предупредить.
        //
        // ЗАПРЕТА ЗДЕСЬ НЕТ И ОКНА ОТКАЗА НЕТ. Смешанный выбор разрешён, а цена
        // названа строкой счёта: «сетей: M — одним номером их не покрыть».
        // Останавливает не окно, а правило данных: `save()` на экране и
        // `oneNetworkPerPhone()` на сервере (К265).
        function openOffersPick() {
            ...
            // Без подсказки под полем: она объясняла ограничение выбора, а
            // ограничения больше нет (К309). Подсказка «ограничений нет» — шум.
            ...
            grid.appendChild(fieldBox('Номер для перевода', phone, {
                required: true,
                wide: true,
                // ⚠ ВТОРАЯ ПОЛОВИНА — СЛОВО В СЛОВО ИЗ `oneNetworkError()`, и это
                // не совпадение. Человек, не прочитавший предупреждение и
                // получивший отказ, встречает одну и ту же фразу дважды и
                // узнаёт её, вместо того чтобы разбираться во второй
                // формулировке одного правила. Меняешь здесь — меняй там же.
                hint: 'Внешний номер партнёра, городской или мобильный. Встанет всем '
                    + 'выбранным офферам: у одного номера может быть только одна сеть — '
                    + 'офферам чужой сети номер меняют в перечне.'
            }));
            ...
            /**
             * Сети отмеченных офферов: СКОЛЬКО их и как зовут единственную.
             *
             * Считаются по номеру, а не по имени (см. шапку окна). Имя берётся
             * у первой отмеченной строки и годится только при `count === 1`:
             * при нескольких сетях называть одну из них значило бы выдать часть
             * за целое.
             */
            function pickedNets() {
                const ids = new Set();
                ...
                for (const o of free) {
                    ...
                    if (!ids.size) name = o.networkName;
                    ids.add(o.networkId);
                }
                return { count: ids.size, name };
            }
            ...
            function pickRow(offer) {
                ...
                box.setAttribute('aria-label', offer.name);
                box.addEventListener('change', () => {
                ...
                // ВСЕ СТРОКИ ОДИНАКОВЫ: флажок доступен, имя не приглушено, в
                // подстроке — имя сети и только оно. Приписка «другая сеть» ушла
                // вместе с выключением (К309): она объясняла, почему нельзя, а
                // теперь можно.
                const who = el('td');
                who.appendChild(el('span', 'ui-table__main', offer.name));
                who.appendChild(el('span', 'ui-table__sub', offer.networkName));

                const priority = el('td', 'ui-table__num');
            ...
            function syncPick() {
                ...
                if (n === 0) {
                    ...
                    const nets = pickedNets();
                    if (nets.count === 1) {
                        // Одна сеть — она просто названа: при одной её полезно
                        // знать, и предупреждать не о чем.
                        countNote.appendChild(document.createTextNode(` · сеть «${nets.name}».`));
                    } else {
                        // ⚠ ПРЕДУПРЕЖДЕНИЕ, А НЕ ЗАПРЕТ. Выбор остаётся, цена
                        // названа: одним номером две сети не покрыть, и номера
                        // правятся в перечне. Сети НЕ перечисляются поимённо —
                        // их может быть сколько угодно, а в строке читается два
                        // имени, дальше строка перестаёт быть строкой.
                        countNote.appendChild(document.createTextNode(' · '));
                        countNote.appendChild(el('b', '', `сетей: ${nets.count}`));
                        countNote.appendChild(document.createTextNode(
                            ' — одним номером их не покрыть, номера правятся в перечне.'));
                    }
                }
        ...
        async function save() {
            ...
            //
            // ⚠⚠ ЗАПРЕТА ОКНА ВЫБОРА БОЛЬШЕ НЕТ — К309, решение владельца 134.
            // Тот не давал отметить офферы разных сетей В ОДНОМ выборе и ДВУХ
            // ДЫР НЕ ЗАКРЫВАЛ: два раздельных выбора с одним номером и правку
            // номера в уже стоящей строке. Обе закрывает эта проверка — и
            // теперь она единственная, что стоит между данными и нарушением.
            // Ослабишь её здесь — на экране не останется ничего.
            //

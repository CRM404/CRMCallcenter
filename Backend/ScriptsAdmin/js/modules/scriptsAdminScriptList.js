// --- scriptsAdminScriptList.js: таблица скриптов (черновики + активный) ---
...
// onOpen(script) — открыть работу с узлами; onChanged() — перезагрузить список после действия.
export function renderScriptList(container, scripts, selectedId, onOpen, onChanged) {
    ...
    container.innerHTML = `
        <table class="sa-script-table">
            <thead>
                <tr><th>Название</th><th>Статус</th><th>Автор</th><th>Действия</th></tr>
            </thead>
            <tbody>${rows}</tbody>
        </table>
    `;
    ...
    container.querySelectorAll('[data-action="activate"]').forEach((btn) => {
        btn.addEventListener('click', async () => {
            const script = scripts.find((s) => s.id === Number(btn.dataset.id));
            const ok = await confirmAction(`Активировать скрипт «${script.title}»? Текущий активный скрипт (если есть) станет черновиком.`);
            if (!ok) return;
            try {
                await updateScript(script.id, { title: script.title, status: 'active', employeeId: script.employeeId });
